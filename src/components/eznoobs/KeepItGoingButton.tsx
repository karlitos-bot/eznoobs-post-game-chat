...
export function KeepItGoingButton({
  ...
  async function handleExtend() {
    ...
    try {
      ...
      // A successful response means the room either reached the cap now or was
      // already there, so this client can safely disable further extension attempts.
  ...
  return (
    <button
      ...
      className="flex min-h-10 items-center gap-1.5 border border-border bg-background/90 px-2.5 font-mono text-[0.6rem] uppercase tracking-[0.1em] text-muted-foreground shadow-lg backdrop-blur-sm transition-all hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-40 sm:px-3 sm:text-[0.62rem] sm:tracking-[0.12em]"
    >
      <Clock3 className="size-3.5" />
      <span className="hidden md:inline">
        {busy ? "Extending…" : canExtend ? "Keep it going +3 min" : "10 min max"}
      </span>
      <span className="md:hidden">+3 min</span>
    </button>
  );
}
